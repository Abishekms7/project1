<?php
    ...
    if(isset($_GET['ids']))
    {
        ...
        {
            $out = array(array("S.No", "Date", "Equipment", "Model", "Brand", "Size", "Received From", "Given To", "Quantity", "Cost", "Amount"));
            ...
            {
                array_push($out, array($s++, sprintf("%02d",$row["date"]).'/'.sprintf("%02d",$row["month"]).'/'.$row["year"], $row["equipment"], $row["model"], $row["brand"], $row["size"], $row["receivedfrom"], $row["givento"], $row["quantity"], $row["cost"], $row["quantity"]*$row["cost"])); 
                // $out.='
                // <tr>
                //     <td>'. $s++ .'</td>
