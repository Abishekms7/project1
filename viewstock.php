<?php       
    ...
    else
    {
        ...
        if($rows=mysqli_fetch_array($result))
        {
            ...
            if(isset($_POST['givento'])) {
                $igivento=mysqli_real_escape_string($stat,$_POST['givento']);    
            } else {
                $igivento="";
            }
    ?>
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                ...
                <form method="post" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>">
                    ...
                    <div class="row">
                        <label class="col-lg-1">Given To</label>
                        <div class="col-lg-2">
                            <select name="givento" class="form-control" required="true">
                            <option value="" disabled>Select Name</option>
                            <option selected="">ALL</option>
                            <?php
                                $query1="SELECT givento from inventory WHERE income=0 GROUP BY givento ORDER BY id";
                                $result1=mysqli_query($stat,$query1); 
                                while($rows1=mysqli_fetch_array($result1)) {
                                    if($igivento==$rows1['givento']) {
                                        ?>
                                        <option selected="" value="<?php echo $rows1['givento'];?>"><?php echo $rows1['givento']; ?></option>
                                        <?php
                                    }
                                    else {
                                        ?>
                                        <option value="<?php echo $rows1['givento'];?>"><?php echo $rows1['givento']; ?></option>
                                        <?php
                                    }
                                }
                            ?>
                ...
                <tbody>
                    ...
                    <?php
                        require_once('db.php');
                        $query1="SELECT * from inventory WHERE income=0 ";
                ...
                <?php
                    if($ids!="") {
                        ?>
                        <a href="excel1.php?ids=<?php echo $ids; ?>">Download In Excel</a>
                        <?php
                    }
                unset($_POST['submit']);
            }
